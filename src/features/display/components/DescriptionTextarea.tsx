...
export const DescriptionTextarea = ({ disabled = false, currentLength, maxLength, maxRows }: Props) => {
  /**
  ...
  return (
    <LabelWrapper label="내용" required>
      <Grid container alignItems="flex-end">
        <Grid item xs={11}>
          <ControlledTextarea<MainBannerDetailFormFields | MainFeedDetailFormFields>
            ...
            disabled={disabled}
            rules={{ required: '내용을 입력하세요', validate: validateLength }}
            maxRows={maxRows}
            minRows={maxRows}
            onInput={handleInput}
            style={{ resize: 'none' }}
