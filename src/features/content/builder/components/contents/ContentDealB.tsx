...
import {
  ...
  DEAL_B_DESC_MAX_NUM,
  DEAL_B_SUBTITLE_MAX_NUM,
  DEAL_B_TITLE_MAX_NUM,
  FORM_KEY,
} from '../../constants';
 ...
 * 상품 B 컴포넌트
 */
type ContentDealBProps = {
  content: ContentModel;
};
export const ContentDealB: VFC<ContentDealBProps> = ({ content }) => {
  ...
  const {
    ...
    goodsColor,
    isGoodsDefaultColor,
  } = formValue[FORM_KEY.CONTENTS] as FormContentDealBModel;
  // 미디어 업로드 정보
  const { backgroundMedia } = formValue[FORM_KEY.MEDIA_UPLOADER] as FormContentDealBUploaderModel;
  ...
  return (
    <>
      <PreviewArcodian
        title="상품 컬렉션 타입"
        previewImage={[
          'story/20220209/6c7156e0-2f2f-45ae-80e4-3399ca49d232.png',
          'story/20220209/4073dd22-0a00-4a43-9f9c-904acade1ed0.gif',
        ]}
      />
                    ...
                    <Box sx={{ width: '100%', mb: 1 }}>
                      <TextController
                        textInputProps={{
                          ...
                          placeholder: `타이틀을 입력하세요 (띄워쓰기 포함 최대 ${DEAL_B_TITLE_MAX_NUM}자 이내)`,
                          inputProps: { max: DEAL_B_TITLE_MAX_NUM },
                          multiline: true,
                        }}
                        textBoldProps={{
                    ...
                    <Box sx={{ width: '100%', mb: 1 }}>
                      <TextController
                        textInputProps={{
                          ...
                          placeholder: `서브타이틀을 입력하세요 (띄워쓰기 포함 최대 ${DEAL_B_SUBTITLE_MAX_NUM}자 이내)`,
                          inputProps: { max: DEAL_B_SUBTITLE_MAX_NUM },
                          multiline: true,
                        }}
                        textBoldProps={{
                    ...
                    <Box sx={{ width: '100%', mb: 1 }}>
                      <TextController
                        textInputProps={{
                          ...
                          placeholder: `디스크립션을 입력하세요 (띄워쓰기 포함 최대 ${DEAL_B_DESC_MAX_NUM}자 이내)`,
                          inputProps: { max: DEAL_B_DESC_MAX_NUM },
                          multiline: true,
                        }}
                        textBoldProps={{
      ...
      <Paper elevation={2} sx={{ p: 3, mt: 10 }}>
        ...
        <List>
          {/* 백그라운드 이미지 */}
          <ListItemWrapper listTitleName="백그라운드 이미지">
            <Box sx={{ width: '100%' }}>
              <Box sx={{ width: '100%' }}>
                <Grid container item>
                  <Box sx={{ width: '100%' }}>
                  ...
                  {bgType === CONTENT_BACKGROUND_TYPE.MEDIA && (
                    <>
                      ...
                      <Grid item xs={7} sx={{ width: '100%' }}>
                        <GuideText
                          title="이미지 업로드 가이드"
                          desc={['권장 이미지 사이즈: (세로형) 400 x 600px', '이미지 파일 형식: png']}
                        />
                      </Grid>
                    </>
                  )}
