...
import { ReturnTypeUseOrderExportPrepareListService } from '@features/order/services';
...
interface Props {
  form: ReturnTypeUseOrderExportPrepareListService['form'];
  providerCombo: ReturnTypeUseOrderExportPrepareListService['providerCombo'];
  mdCombo: ReturnTypeUseOrderExportPrepareListService['mdCombo'];
  brandCombo: ReturnTypeUseOrderExportPrepareListService['brandCombo'];
}
...
/**
 * 송장 등록 대기 검색폼 component
 */
export const OrderExportPrepareSearchForm = ({
  form: {
    formMethod: {
      formState: { errors },
      watch,
  ...
  const isSelectAll = OrderStatusExportableOptions.length === orderStepListValues.filter((item) => !!item).length;
              ...
              <FormControlStyled fullWidth>
                ...
                <FormGroup>
                  <CheckboxWrapperStyled>
                    <StatusOptionGroup
                      buttonLabel="출고전 상태"
                      options={OrderStatusExportableOptions}
                      startIndex={0}
...
const CheckboxWrapperStyled = styled(Box)`
  display: flex;
  align-items: center;
`;
